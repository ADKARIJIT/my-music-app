...
async function getsong(folder) {
    let response = await fetch(`http://10.1.160.82:3000/song/${folder}/`);
...
async function displayalbum(){
    let a = await fetch(`http://10.1.160.82:3000/song/`);
    // console.log(response)
     ...
     for(let items = 0; items<array.length ; items++){
        if(array[items].href.includes("%5Csong%5C")){
            ...
            console.log(fold)
            let b = await fetch(`http://10.1.160.82:3000/song/${fold}/info.json`);
            // console.log(response)
